...
for fold = 1:K
    ...
    if sum(isnan(feat_weight)) > 0
        [feat_rank, feat_weight] = relieff(table2array(X_train_z),y_train,2*K); % backup for noisy data
    end
...
for fold = 1:K
    ...
    if sum(isnan(feat_weight)) > 0
        [feat_rank, feat_weight] = relieff(table2array(X_train_z),y_train,4*K); % backup for noisy data
    end
...
save('ws_perf_Ensemble_dataset2.mat','perf_leaky_full','perf_leaky_ablFS','perf_leaky_ablTT','perf_subj')