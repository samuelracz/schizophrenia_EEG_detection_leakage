...
fnames = dir('Dataset1_processed/*.gdf'); % replace with appropriate path to processed MSU data
ns = length(fnames);
subjIDs = {fnames.name};
...
for subj = 1:ns
    ...
    for t = 1:Nt
        ...
        I1 = func_organize_spectrograms_data1(S_ch,freq,xt,[0, 45]);
        I2 = func_organize_spectrograms_data1(CW_ch,cf,xt_cwt,[1-eps, 45]);
        I3 = func_organize_spectrograms_data1(DWV_ch,bf,xt_dwv,[1-eps, 45]);
    ...
    if ~exist('Dataset1_CNN_features','dir')
        mkdir('Dataset1_CNN_features')
    end
    save(['Dataset1_CNN_features/' fname_out],'feat_struct')
