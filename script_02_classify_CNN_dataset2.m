...
fnames = dir('Dataset2_CNN_features/*.mat');
ns = length(fnames);
...
for fold = 1:K
    ...
    [YPred_train, scores_train] = classify(netFold, imdsTrain); % classify train examples
    trainAll = readall(imdsTrain);
    YTrue_train = categorical(cell2mat(trainAll(:,2)));
    Ytr_leaky{fold} = YTrue_train;
    Ypr_leaky{fold} = YPred_train;
    acc_train_leaky(fold) = mean(YPred_train==YTrue_train);
...
fprintf('Leaky 10-Fold CV Mean Train Accuracy: %.2f%% (SD: %.2f%%)\n', mean(acc_train_leaky)*100, std(acc_train_leaky)*100);
...
save('ws_perf_CNN_dataset2.mat','perf_leaky','perf_subj')